...
class Score(ppb.Sprite):
    image = ppb.Text("0", font=FONT, color=(255, 255, 255))

    def on_pre_render(self, event, signal):
        self.image = ppb.Text(str(event.score), font=FONT, color=(255, 255, 255))
...
class Game(ppb.Scene):
    ...
    def __init__(self, **kwargs):
        ...
        self.add(Score(position=ppb.Vector(0, 15), layer=10))
...
class Scoring(ppb.systemslib.System):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.score = 0
        self.engine.register(ppb.events.PreRender, self.set_score)
...
ppb.run(starting_scene=TitleScreen, resolution=(600, 800), systems=[Scoring])
